...
@cross_origin()
def getimage(id):
    filename = BASE_PATH_TRAIN + id + "/0.jpg"
...
def registerUser():
    if request.is_json:
        try:
            ...
            response = {'RespMsg':'Registration success'}
            # Watcher.add_new(1)

            # Inserting into database
            mycursor = mydb.cursor()
            mycursor.execute("UPDATE users SET isnew=1 WHERE id = " + userid)
            mydb.commit()
...
def imageRecognizer():
    if request.is_json:
       try:
            ...
            mycursor = mydb.cursor()
            mycursor.execute("SELECT * FROM users WHERE id = "+person)
